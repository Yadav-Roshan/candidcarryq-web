...
import { countryCodes, findDefaultCountry, CountryCode } from "@/lib/country-codes"
...
export function PhoneInput({
    ...
    if (value !== `${selectedCountry.dial_code}${localPhoneNumber}`) {
      ...
      if (value.startsWith('+')) {
        ...
        if (matchingCountry) {
          ...
          setLocalPhoneNumber(value.substring(matchingCountry.dial_code.length));
          return;
        }
      }
  ...
  const handleCountryChange = (countryCode: string) => {
    const country = countryCodes.find(c => c.code === countryCode) || defaultCountry;
